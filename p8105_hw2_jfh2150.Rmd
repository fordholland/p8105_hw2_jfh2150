...
The `Mr. Trash Wheel` data comprise `r ncol(df_trash_data)` columns and `r nrow(df_trash_data)` observations. Key features of this dataset include `weight_tons` and `volume_cubic_yards`, the weight and volume, respectively, of trash collected per month. The number of homes powered by incinerating the collected trash, `homes_powered`, is also an interesting feature that demonstrates the impact that Mr. Trash Wheel has on the Baltimore community. These data also contain quantities of certain itemized trash objects, such as the number of sports balls collected. In 2017 for example, the median number of balls collected per month was `r df_trash_data %>% filter(year == 2017) %>% summarize(median = median(sports_balls))`.


The precipitation data describe the total precipitation in Baltimore per month. There are `r nrow(df_precipitation)` observations, representing precipitation for all months in 2017 and 2018. Notably, the total precipitation in 2018 was `r df_precipitation %>% filter(year == 2018) %>% summarize(sum = sum(total_precipitation))` inches, more than twice the `r df_precipitation %>% filter(year == 2017) %>% summarize(sum = sum(total_precipitation))` inches received in 2017. In fact, 2017 precipitation only exceeded 2018 in 2 months.
