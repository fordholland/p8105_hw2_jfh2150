...
impact that Mr. Trash Wheel has on the Baltimore community. These data
also contain quantities of certain itemized trash objects, such as the
number of sports balls collected. In 2017 for example, the median number
of balls collected per month was 8.
...
70.33 inches, more than twice the 32.93 inches received in 2017. In
...
df_precipitation %>%
  group_by(year) %>%
  summarise(mean(total_precipitation))
```

    ## # A tibble: 2 x 2
    ##    year `mean(total_precipitation)`
    ##   <dbl>                       <dbl>
    ## 1  2017                        2.74
    ## 2  2018                        5.86
...
``` r
# notably, precipitation in 2017 only exceeded 2018 values in 2 months
df_precipitation %>%
  pivot_wider(names_from = year, values_from = total_precipitation) %>%
...
```
    ...
    ##    <chr>      <dbl>  <dbl>      <dbl>
    ##  1 January     2.34   0.94     -1.4  
    ##  2 February    1.46   4.8       3.34 
    ##  3 March       3.57   2.69     -0.880
    ##  4 April       3.99   4.69      0.7  
    ##  5 May         5.64   9.27      3.63 
    ##  6 June        1.4    4.77      3.37 
    ##  7 July        7.09  10.2       3.11 
    ##  8 August      4.44   6.45      2.01 
    ##  9 September   1.95  10.5       8.52 
    ## 10 October     0      2.12      2.12 
    ## 11 November    0.11   7.82      7.71 
    ## 12 December    0.94   6.11      5.17
...
Note start years of snp and unemp rate
...
president. There are 822 per-month observations.
...
Popularity of name “Olivia” for females by ethnicity and
year.
...
| ethnicity                  | rank | 2011    | 2012   | 2013   | 2014   | 2015   | 2016   |
| :------------------------- | ---: | :------ | :----- | :----- | :----- | :----- | :----- |
...
Most popular male name by ethnicity and
year.
...
<img src="p8105_hw2_jfh2150_files/figure-gfm/unnamed-chunk-4-1.png" width="90%" />
